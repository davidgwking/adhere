  ...
  if (Array.isArray(schemas)) {
    schemas.forEach(function (schema, index) {
      if (arr[index] === undefined) {
        ...
        self.errors = self.errors.concat([[[self.namespace, index].join('.'), 'expected data'].join(': ')]);
      }
    });
    ...
    arr.forEach(function (val, index) {
      ...
      if (schemas[index] === undefined) {
        ...
        result = {
          valid: false,
          errors: [[[self.namespace, index].join('.'), 'unexpected schema'].join(': ')]
        };
      }
...
ArrayValidator.prototype.maxItems = function maxItems(arr) {
  if (arr.length > this.schema.maxItems) {
    this.valid = false;
    this.errors.push([this.namespace, 'exceeds maximum items ' + this.schema.maxItems].join(': '));
  }
};
...
ArrayValidator.prototype.minItems = function minItems(arr) {
  if (arr.length < this.schema.minItems) {
    this.valid = false;
    this.errors.push([this.namespace, 'exceeds minimum items ' + this.schema.minItems].join(': '));
  }
};
...
ArrayValidator.prototype.uniqueItems = function uniqueItems(arr) {
  if (unique(arr, this.comparator).length !== arr.length) {
    this.valid = false;
    this.errors.push([this.namespace, 'violates unique items'].join(': '));
  }
};
