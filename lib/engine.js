...
function validate(val, schema, definitions, namespace, parent) {
  ...
  // resolve self in case of self-referencing
  schema = utils.resolveSubschema(schema, definitions);
  ...
  if (schema.allOf) {
    ...
    var satisfiesAllOf = schema.allOf.reduce(function (memo, schema) {
      var resolvedSchema = utils.resolveSubschema(schema, definitions);
      var result = validate(val, resolvedSchema, definitions);
  ...
  if (schema.anyOf) {
    var satisfiesAnyOf = schema.anyOf.reduce(function (memo, schema) {
      var resolvedSchema = utils.resolveSubschema(schema, definitions);
      return memo || validate(val, resolvedSchema, definitions).valid;
    }, false);
  ...
  if (schema.oneOf) {
    var numSatisfied = schema.oneOf.reduce(function (memo, schema) {
      var resolvedSchema = utils.resolveSubschema(schema, definitions);
      if (validate(val, resolvedSchema, definitions).valid) memo = memo + 1;
      return memo;
    }, 0);
  ...
  // test not
  if (schema.not) {
    var resolvedSchema = utils.resolveSubschema(schema.not, definitions);
    var satisfiesNot = !validate(val, resolvedSchema, definitions).valid;
