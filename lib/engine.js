...
function validate(val, schema, namespace, parent) {
  ...
  var validator = types.reduce(function (memo, type) {
    ...
    var validator = validators[type](namespace, schema, module.exports);
  ...
  if (schema.allOf) {
    ...
    var satisfiesAllOf = schema.allOf.reduce(function (memo, schema) {
      var result = validate(val, schema);
  ...
  if (schema.anyOf) {
    var satisfiesAnyOf = schema.anyOf.reduce(function (memo, schema) {
      return memo || validate(val, schema).valid;
    }, false);
  ...
  if (schema.oneOf) {
    var numSatisfied = schema.oneOf.reduce(function (memo, schema) {
      if (validate(val, schema).valid) memo = memo + 1;
      return memo;
    }, 0);
  ...
  // test not
  if (schema.not) {
    var satisfiesNot = !validate(val, schema.not).valid;
